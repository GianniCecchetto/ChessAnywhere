...
def draw_chessboard(parent, size=8, square_size=70, board=None, playable_square=None):
    """
        ...
        for col in range(size):
            ...
            if playable_square:
                if playable_square[row][col] == "X":
                    fill_color = "#FF0000"
                elif playable_square[row][col] == "O":
                    fill_color = "#0026FF"
                elif playable_square[row][col] == "P":
                    fill_color = "#00FBFF"

            canvas.create_rectangle(x1, y1, x2, y2, fill=square_color, outline="", width=OUTLINE_WIDTH)
            if playable_square and playable_square[row][col] != ".":
                canvas.create_rectangle(x1 + 6, y1 + 6, x2 - 6, y2 - 6, fill=fill_color, width=OUTLINE_WIDTH, outline="gray")
